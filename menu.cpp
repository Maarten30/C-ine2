	...
	for (Usuario a: users)
	{
		...
		if (nomUsuario.compare(a.getNombreUs())==0 && dni.compare(a.getDNI())==0)
		{
			...
			*us = a;
			...
			int op = 0;
			char* cart1 = nombreCine(); //AquÌ pide al usuario el nombre del cine
			a.imprimirMenu();
			...
			if(repetido == true)
			{
					do
					{
						...
						char* cart2 = nombreCine(); //AquÌ pide al usuario el nombre del cine
						us ->imprimirMenu();
						cin >> op;
...
//	{
		...
		cout << "La peli m√°s taquillera es: " << peli.getTitulo() << endl;
	}
//}
